---
name: interview
description: Plan a task through an interactive interview process
...
Help the user plan tasks through a structured interview process.

## Flow: Plan task (`/interview <description>`)
...
- Use the `AskUserQuestion` tool to ask each interview question. Do not use plain text output for questions, unless the user asks to chat about a question.
...
- Update the plan incrementally so the user can see progress and correct course.
- Use plain, direct language in the plan — no filler.
...
After the user confirms the interview is complete, launch THREE review agents in parallel. Each receives the current plan and has read access to the codebase.

### 1. Software Review (Agent subagent_type="general-purpose")

Provide: plan contents + instruction to explore relevant parts of the codebase.
...
- Flag design decisions the plan leaves open that should be resolved before implementation — e.g., "the plan says 'add validation' but doesn't specify where: at the handler level (consistent with existing pattern) or at the service level"
...
<Choices the plan doesn't resolve that will affect implementation>
...
Provide: plan contents + instruction to explore the broader application structure.
...
Provide: plan contents + instruction to explore the codebase structure for parallelization assessment.
...
**Examples:**
- "The plan says 'add validation' but doesn't specify where — handler or service level?"
...
1. **Present all findings to the user**, organized by category. For Category A items, be explicit: "These need your input before we can finalize the plan."
...
2. **If there are Category A items → enter a focused interview.**
   ...
   - Update plan after each decision
...
4. **If there are NO Category A items**, skip straight to populating Implementation notes with Category B findings, show the final plan, and confirm.
